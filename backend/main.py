...
class BackUtils:
    def __init__(self, compt_as_date: datetime.date) -> None:
        self.compt_as_date = compt_as_date

    def permitir_ser_declarado(self, filtered_cnpjs, is_permited):
        ...
        for cnpj in filtered_cnpjs:
            permitir_ok = COMPT_ORM_OPERATIONS.update_fieldict_from_cnpj_compt(
                cnpj, self.compt_as_date, {'pode_declarar': is_permited})
        if permitir_ok:
            return True
        else:
            return False
    ...
    def obtem_dados_empresa(self) -> list:
    ...
    def return_dados_gerais(self) -> pd.DataFrame():
        GERAL = COMPT_ORM_OPERATIONS.df_join_empresas(self.compt_as_date)
        return GERAL
